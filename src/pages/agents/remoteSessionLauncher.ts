...
export async function openRemoteSessionPopup({
    ...
    const request: StartRemoteSessionRequest = {
        ...DEFAULT_PARAMS,
        ...(kind ? { kind } : {}),
        ...(transport ? { transport } : {}),
        ...(quality ? { quality } : {}),
    ...
    try {
        ...
        throw new Error(`Falha ao iniciar sessão remota: ${err instanceof Error ? err.message : String(err)}`);
    }
    ...
    if (!natsSubject) {
        ...
        throw new Error('Sessão remota criada sem subject NATS. Verifique a configuração do backend.');
    }
    ...
    try {
        const turn = await remoteSessionsApi.getTurnCredentials(agentId, session.sessionId);
        turnCredentials = {
            urls: turn.urls,
