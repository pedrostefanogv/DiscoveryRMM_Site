...
export function TextArea({ label, className = '', id, ...props }: TextAreaProps) {
  ...
  return (
    <div className="space-y-1">
      ...
      <textarea
        id={textId}
        className={`w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-200 placeholder-slate-500 outline-none transition-colors focus:border-primary/50 focus:ring-1 focus:ring-primary/30 ${className}`}
        rows={3}
        {...props}
      />
    </div>
