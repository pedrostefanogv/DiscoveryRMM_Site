...
/**
 * Detalhe somente-leitura de um template de chamado (inclusive excluído):
 * permite conferir os dados, o questionário e os valores padrão sem abrir o
...
export function TemplateDetailsModal({
  ...
  const defaults = useMemo(() => {
    if (!template) return [] as { key: string; label: string; value: string }[];
  ...
  return (
    <Modal open onClose={onClose} title={template.name} maxWidth="max-w-2xl">
