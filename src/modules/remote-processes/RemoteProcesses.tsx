...
import { useProcessesStream, type ProcessInfo, type ServiceInfo } from './useProcessStream';
...
type Panel = 'processes' | 'services';

const processStateColor = (state: string) => {
    switch (state) {
...
export function RemoteProcesses({ natsSubject, natsUrl, jwt }: RemoteProcessesProps) {
    const [panel, setPanel] = useState<Panel>('processes');
    const [processes, setProcesses] = useState<ProcessInfo[]>([]);
    const [services, setServices] = useState<ServiceInfo[]>([]);
    const [loading, setLoading] = useState(false);
    const [busy, setBusy] = useState(false);
    const [search, setSearch] = useState('');
    const [menu, setMenu] = useState<{ x: number; y: number; process?: ProcessInfo; service?: ServiceInfo } | null>(null);
    ...
    const runAction = useCallback(async (action: string, body: Record<string, unknown>, successMsg: string) => {
        ...
        try {
            ...
            if (res.success) {
                notify(successMsg);
                if (panel === 'processes') await loadProcesses();
    ...
    }, [stream.send, notify, panel, loadProcesses, loadServices]);
    ...
    return (
        <div className="h-full flex flex-col min-h-0 bg-slate-950" onContextMenu={(e) => e.preventDefault()}>
            {/* Sub-tabs: Processos | Serviços */}
            <div className="flex items-center gap-1 px-3 py-2 border-b border-slate-800">
                <button
                    className={`px-3 py-1 rounded text-sm ${panel === 'processes' ? 'bg-slate-700 text-white' : 'text-slate-400 hover:text-slate-200'}`}
                    onClick={() => setPanel('processes')}
                >
                    🗔 Processos ({processes.length})
                </button>
                <button
                    className={`px-3 py-1 rounded text-sm ${panel === 'services' ? 'bg-slate-700 text-white' : 'text-slate-400 hover:text-slate-200'}`}
                ...
                />
                <button
                    onClick={() => { if (panel === 'processes') loadProcesses(); else loadServices(); }}
                            ...
                            {filteredServices.map((s) => (
                                ...
                                >
                                    ...
                                    <td className="px-3 py-1">
                                        <span className={`inline-flex items-center gap-1 ${processStateColor(s.state)}`}>
